...
function hostOnly(host:string){return host.toLowerCase().split(":")[0];}
export async function proxy(request:NextRequest){
  ...
  if(hostname.endsWith(`.${configured}`)){const slug=hostname.slice(0,-1*(configured.length+1)).split(".")[0];if(slug&&slug!=="www"&&slug!=="app"){const rewrite=request.nextUrl.clone();rewrite.pathname=`/store/${encodeURIComponent(slug)}${path==="/"?"":path}`;return NextResponse.rewrite(rewrite);}}
