...
async function getProductVariant(db:ReturnType<typeof createSupabaseAdminClient>,productId:string,raw:FormDataEntryValue|null){
  const variantId=String(raw||"")||null;
  if(!variantId)return null;
  const {data}=await db.from("product_variants").select("id,name").eq("id",variantId).eq("product_id",productId).maybeSingle();
  if(!data)throw new Error("INVALID_VARIANT");
  return data;
}
...
export async function importLicensesAction(formData:FormData){const {merchantId}=await requireMerchant(); const productId=String(formData.get("productId")); const {db,product}=await getOwnedProduct(productId,merchantId); if(product.type!=="license") throw new Error("WRONG_PRODUCT_TYPE"); const variant=await getProductVariant(db,productId,formData.get("variantId")); const variantId=variant?.id??null; let query=db.from("license_pools").select("id").eq("product_id",productId); query=variantId?query.eq("variant_id",variantId):query.is("variant_id",null); let {data:pool}=await query.limit(1).maybeSingle(); if(!pool){const r=await db.from("license_pools").insert({product_id:productId,variant_id:variantId,name:variant?.name||"Product default"}).select("id").single();pool=r.data;} if(!pool) throw new Error("POOL_CREATE_FAILED"); const keys=String(formData.get("keys")||"").split(/\r?\n/).map(x=>x.trim()).filter(Boolean); if(!keys.length) throw new Error("NO_LICENSES"); const rows=keys.map(key=>({pool_id:pool!.id,key_value:key,key_hash:sha256(key),status:"available"})); const {error}=await db.from("licenses").upsert(rows,{onConflict:"pool_id,key_hash",ignoreDuplicates:true}); if(error) throw new Error(error.message); revalidatePath(`/dashboard/products/${productId}`);}

export async function importAccountsAction(formData:FormData){const {merchantId}=await requireMerchant(); const productId=String(formData.get("productId")); const {db,product}=await getOwnedProduct(productId,merchantId); if(product.type!=="account") throw new Error("WRONG_PRODUCT_TYPE"); const variant=await getProductVariant(db,productId,formData.get("variantId")); const variantId=variant?.id??null; let query=db.from("account_pools").select("id").eq("product_id",productId); query=variantId?query.eq("variant_id",variantId):query.is("variant_id",null); let {data:pool}=await query.limit(1).maybeSingle(); if(!pool){const r=await db.from("account_pools").insert({product_id:productId,variant_id:variantId,name:variant?.name||"Product default"}).select("id").single();pool=r.data;} if(!pool) throw new Error("POOL_CREATE_FAILED"); const lines=String(formData.get("accounts")||"").split(/\r?\n/).map(x=>x.trim()).filter(Boolean); if(!lines.length)throw new Error("NO_ACCOUNTS"); const rows=lines.map(line=>{let payload:unknown; try{payload=JSON.parse(line);}catch{const [username,...rest]=line.split(":");payload={username,password:rest.join(":")};} const raw=JSON.stringify(payload); return {pool_id:pool!.id,encrypted_payload:encryptSecret(raw),fingerprint:sha256(raw),status:"available"};}); const {error}=await db.from("account_inventory").upsert(rows,{onConflict:"pool_id,fingerprint",ignoreDuplicates:true}); if(error) throw new Error(error.message); revalidatePath(`/dashboard/products/${productId}`);}

export async function uploadDigitalFileAction(formData:FormData){const {merchantId}=await requireMerchant(); const productId=String(formData.get("productId")); const {db,product}=await getOwnedProduct(productId,merchantId); if(product.type!=="digital_file") throw new Error("WRONG_PRODUCT_TYPE"); const variant=await getProductVariant(db,productId,formData.get("variantId")); const file=formData.get("file"); if(!(file instanceof File)||file.size===0) throw new Error("NO_FILE"); const limits=await getPlanLimits(merchantId); const usage=await getMerchantUsage(merchantId); if(usage.storage_mb+Math.ceil(file.size/1024/1024)>limits.storage_mb) throw new Error("PLAN_LIMIT:storage_mb"); const path=`${product.store_id}/${productId}/${crypto.randomUUID()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,"_")}`; const {error}=await db.storage.from("digital-files").upload(path,file,{contentType:file.type||"application/octet-stream",upsert:false}); if(error) throw new Error(error.message); const {error:rowError}=await db.from("digital_files").insert({product_id:productId,variant_id:variant?.id??null,storage_path:path,original_name:file.name,mime_type:file.type,size_bytes:file.size,max_downloads:formData.get("maxDownloads")?Number(formData.get("maxDownloads")):null}); if(rowError){await db.storage.from("digital-files").remove([path]);throw new Error(rowError.message);} await db.rpc("increment_usage",{p_merchant:merchantId,p_metric:"storage_mb",p_amount:Math.ceil(file.size/1024/1024)}); revalidatePath(`/dashboard/products/${productId}`);}
